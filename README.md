...
### Installation
**For Ubuntu-16.04.3**  
参考：[How to Install NS-2.35 in Ubuntu-13.10 / 14.04 (in 4 easy steps)](http://installwithme.blogspot.com/2014/05/how-to-install-ns-2.35-in-ubuntu-13.10-or-14.04.html)  
[Installing Network Simulator 2 (NS2) on Ubuntu 14.04](https://www.howtoforge.com/tutorial/ns2-network-simulator-on-ubuntu-14.04/)  

**STEP1:** Setting up the Prerequisites(Optional)
	...
	udo apt-get install perl xgraph libxt-dev libx11-dev libxmu-dev
...
将`~/path_to/ns-allinone-2.35/ns-2.35/linkstate/ls.h` 137行
	...
	cd ~/path_to/ns-allinone-2.35/
	sudo ./instlall
大约花费15分钟.

**STEP3:** Set the Environment Variables(`path_to`为ns2路径)
...
ns2 current release 2.35(released Nov 4 2011) 
推荐下载allinone包，包含所有必备包。 
Download from sourceforge: ns-allinone-2.35.tar.gz or my repo: Here
...
For Ubuntu-16.04.3 
参考：How to Install NS-2.35 in Ubuntu-13.10 / 14.04 (in 4 easy steps) 
...
udo apt-get install perl xgraph libxt-dev libx11-dev libxmu-dev
sudo apt-get install gcc-4.4
STEP2: Extract and Install NS(path_to为ns2路径)
...
STEP3: Set the Environment Variables(path_to为ns2路径)
...
The NS Manual PDF in isi or in my repo here or HTML format.
...
ns simple-rtp.tcl in ns-2/tcl/ex/newmcast: RTP. Black for data packets, green for RTCP reports, cream for join messages, purple for prune messages.
ns srm-demo.tcl in ns-2/tcl/ex: an SRM (Scalable Reliable Multicast) demo. Comments in srm-demo.txt.
Five tests demonstrate the behaviour of ns’ SRM implementation: Scripts are: here
Traffic
